...
parser.add_argument('--num_uncertain', type=int, default=50, metavar='N',
                    help='num of uncertain sample each epoch (default: 10)')
...
parser.add_argument('--char_layers', type=int, default=5,
                    help='# of character-level convolution layers (default: 3)')
parser.add_argument('--word_layers', type=int, default=4,
...
parser.add_argument('--word_nhid', type=int, default=350,
...
parser.add_argument('--save', type=str, default='model_active.pt',
...
charset = Charset()
vocab = Vocabulary()
vocab.load("data/new/vocab.txt")
tag_set = Index()
tag_set.load("data/new/tag2id.txt")
relation_labels = Index()
relation_labels.load('data/new/relation_labels.txt')

train_data = load('data/new/train.pk')
test_data = load('data/new/test.pk')
...
word_embeddings = torch.tensor(np.load("data/new/word2vec.vectors.npy"))
...
if os.path.exists("model_active.pt"):
    model=torch.load('model_active.pt')
else:
...
train_data_current = [] 
train_data_current = train_data[0:1]

def train(data_groups,data_array):
    print("train: " + str(len(data_array)))
    model.train()
    total_loss = 0
    count = 0
    sampler = GroupBatchRandomSampler(data_groups, args.batch_size, drop_last=False)
    for idx, batch_indices in enumerate(sampler):
        sentences, tokens, targets, lengths = get_batch(batch_indices, data_array)
        #print(batch_indices)
        optimizer.zero_grad()
...
def evaluate(data_groups,data_array):
    model.eval()
    total_loss = 0
    count = 0
    TP = 0
...
def strategy(data_groups,data_array,num_uncertain):
    model.eval()
    total_loss = 0
    count = 0
    TP = 0
    ...
    with torch.no_grad():
        for batch_indices in GroupBatchRandomSampler(data_groups, args.batch_size, drop_last=False):
            ...
            #print(output.size())
            tp, tp_fp, tp_fn = measure(output, targets, lengths)
            sorted_output = output.sort(descending=True)[0]

            #print(sorted_output[0][:, 0] - sorted_output[0][:, 1])
            for i in range(output.size()[0]):
                #print(sorted_output[i].size())
                #print(output.size()[1])
                
                score = sum(sorted_output[i][:, 0] - sorted_output[i][:, 1])/output.size()[1]
                score_array.append([score.tolist(), batch_indices[i]])
                #print(score)
            #print(targets)
            TP += tp
            TP_FP += tp_fp
            ...
            total_loss += loss.item()
            count += len(targets)
    #print(sorted(score_array))
    score_array_sorted = sorted(score_array)
    print(len(train_data_current))
    print(len(train_data))
    indexes=[]
    for i in range(num_uncertain):
        #print(score_array_sorted[i][1])
        indexes.append(score_array_sorted[i][1])
        train_data_current.append(data_array[score_array_sorted[i][1]]) 
...
if __name__ == "__main__":
    ...
    try:
        ...
        for epoch in range(1, args.epochs+1):
            ...
            train_data_current = [] 
            train_data_current = train_data[0:1]
            num_pool = len(train_data)
            num_uncertain = args.num_uncertain
            i = 0
            lr_iter = 0
            while(num_pool):
                i = i + 1
                if (num_pool < num_uncertain):
                    num_uncertain = num_pool
    
                train_data_groups_current = group(train_data_current, [10, 20, 30, 40, 50, 60])
                train(train_data_groups_current, train_data_current)
    
                train_data_groups = group(train_data, [10, 20, 30, 40, 50, 60])
                strategy(train_data_groups, train_data, num_uncertain)
    
                num_pool = num_pool - num_uncertain
    
                val_loss, precision, recall, f1 = evaluate(val_data_groups, val_data)
    
                elapsed = time.time() - start_time
                print("-" * 118)
                print("upstreamEpoch {:2d} ".format(epoch))
                print("| End of Epoch {:2d} | Elapsed Time {:s} | Validation Loss {:5.3f} | Precision {:5.3f} "
                      "| Recall {:5.3f} | F1 {:5.3f} |".format(i, time_display(elapsed),
